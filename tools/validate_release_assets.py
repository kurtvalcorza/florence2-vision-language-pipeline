"""Static release-asset validation for the Florence-2-large vision-language DIMER pipeline.
...
EXPECTED_MODEL_ID = "florence-community/Florence-2-large"
...
# sample is pinned to (samples.py CORPUS_REVISION) and the rejected microsoft/Florence-2-large pin recorded in the
# weight provenance document.
KNOWN_SHAS: frozenset[str] = frozenset(("c4a74bbd39f2df314752e7e6026649a39d365cbb", "21a599d414c4d928c9032694c424fb94458e3594"))
...
EXPECTED_OUTPUTS = (
    "outputs/florence2_vision_language_train.csv",
    "outputs/florence2_vision_language_example_line.png",
    "outputs/florence2_vision_language_input_manifest.json",
    "outputs/florence2_vision_language_evaluation_report.json",
    "outputs/florence2_vision_language_examples",
    "outputs/florence2_vision_language_adapter",
    "outputs/florence2_vision_language_result.json",
)
...
CODE_MARKERS = (
    ...
    "report = evaluation_report(list(results.values()), drawing_reference, sample_kind=",
...
MARKDOWN_MARKERS = (
    "**Capability:** prompt-selected vision-language tasks (caption, detection and OCR demonstrated) and bounded supervised fine-tuning of the `<OCR>` task's last decoder layers",
    "**MIT** licence",
    "**task prompt token**",
    "**deterministic beam search**",
    "**adaptation of one task on transcribed lines**",
    ...
    "the other task tokens in `TASKS` are exposed but not demonstrated",
...
EXTERNAL_WEIGHT_BYTES: dict[int, str] = {
    210_579_166: "Teklia/Belfort-line@c4a74bb default/test/0000.parquet (dataset)",
}
EXTERNAL_WEIGHT_DIGESTS: dict[str, str] = {
    "b7e1dd684691a0eedb63a609311f4964e7732e5c1a8d254fe4e1293a8cd0964d": "SAMPLE_DIGEST: dataset_digest of the default seeded Belfort-line draw",
}
