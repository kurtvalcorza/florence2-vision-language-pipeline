...
from florence2_vision_language_pipeline import pipeline as pl
from florence2_vision_language_pipeline import samples as sm
...
def _echo_runner(image, prompt, task, max_new_tokens, num_beams):
    """An injected single-image runner that returns the line it was shown (a side table by pixel digest)."""
    text = _echo_runner.table.get(image_digest(image), "")
    return {"text": text, "parsed": text, "new_tokens": 5}


def _echo_batch_runner(images, max_new_tokens, num_beams):
...
def test_evaluate_scores_hypotheses_from_the_runner_and_flags_truncation():
    ...
    pipe = Florence2Pipeline(_echo_runner, "cpu", _batch_runner=_echo_batch_runner)
    ...
    assert result["model_revision"] == MODEL_REVISION and result["seconds"] >= 0.0 and result["num_beams"] == 3
    with pytest.raises(ValueError, match="1..5000"):
        pipe.evaluate([])
    single = Florence2Pipeline(_echo_runner, "cpu")  # no batch runner: transcribe falls back to run() per image
...
def test_adapt_and_artifacts_require_a_loaded_model(forbid_model_imports):
    pipe = Florence2Pipeline(_echo_runner, "cpu")
...
def _manifest(tmp_path, **overrides):
    ...
    manifest = {
        ...
        "adapter": {"best_epoch": 1},
        "tensors": ["model.language_model.decoder.layers.11.fc2.weight", "model.language_model.decoder.layernorm_embedding.weight"],
...
def test_check_artifact_manifest_accepts_a_consistent_manifest_and_refuses_each_deviation(tmp_path):
    ...
    for name in ("model.language_model.decoder.layers.7.fc2.weight", "model.language_model.encoder.layers.11.fc2.weight", "model.vision_tower.blocks.0.x", "model.language_model.shared.weight", "lm_head.weight"):
...
def test_trainable_names_selects_the_last_layers_and_the_norm():
    ...
    class _Model:
        def named_parameters(self):
            names = ["model.vision_tower.blocks.0.x", "model.multi_modal_projector.x", "model.language_model.shared.weight", "model.language_model.encoder.layers.11.x", "model.language_model.decoder.layers.0.x", "model.language_model.decoder.layers.7.x", "model.language_model.decoder.layers.8.x", "model.language_model.decoder.layers.11.fc1.weight", "model.language_model.decoder.layernorm_embedding.weight", "lm_head.weight"]
            return [(n, _Param()) for n in names]

    assert pl._trainable_names(_Model()) == ["model.language_model.decoder.layers.8.x", "model.language_model.decoder.layers.11.fc1.weight", "model.language_model.decoder.layernorm_embedding.weight"]
    assert pl._TRAINABLE_FIRST_LAYER == 8 and len(pl._TRAINABLE_PREFIXES) == 5
