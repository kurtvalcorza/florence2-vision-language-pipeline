...
from florence2_vision_language_pipeline.pipeline import _TRAINABLE_PREFIXES
...
@pytest.fixture(scope="module")
def pipe():
    return Florence2Pipeline.from_pretrained(weights_dir=DEFAULT_WEIGHTS_DIR)
...
def test_model_facts_batched_recognition_and_frozen_evaluation(pipe, records):
    ...
    encoded = pipe._processor(text=["<OCR>", "<OCR>"], images=[records[0]["image"], records[1]["image"]], return_tensors="pt", padding=True)
    assert tuple(encoded["input_ids"].shape) == (2, OCR_PROMPT_TOKENS) and bool(encoded["attention_mask"].all())
    single = [pipe.run(r["image"], "<OCR>", max_new_tokens=64) for r in records[:3]]
    ...
    # the batch shape can round a beam differently; the texts agree on most lines
    assert sum(" ".join(str(s["result"]).split()) == b["text"] for s, b in zip(single, batched, strict=True)) >= 2
...
def test_one_epoch_adaptation_and_artifact_round_trip(pipe, records, tmp_path):
    ...
    assert result["n_trainable"] == ADAPTER_PARAMETERS and result["n_total"] == PARAMETER_COUNT and result["first_trainable_layer"] == 8
    ...
    assert not any(n.startswith(("model.vision_tower", "model.multi_modal_projector", "model.language_model.shared", "model.language_model.encoder", "lm_head")) for n in result["trainable_names"])
    ...
    reloaded = Florence2Pipeline.from_artifact(artifact, weights_dir=DEFAULT_WEIGHTS_DIR)
...
def test_load_artifact_refuses_a_tensor_set_that_differs_from_the_recorded_configuration(pipe, records, tmp_path):
    ...
    with pytest.raises(ValueError, match="does not match its recorded configuration"):
        Florence2Pipeline.from_artifact(fewer, weights_dir=DEFAULT_WEIGHTS_DIR)
    extra = tmp_path / "extra"
    shutil.copytree(artifact, extra)
    tensors = load_file(str(extra / "adapter.safetensors"))
    tensors["model.language_model.decoder.layernorm_embedding.zz_extra"] = torch.zeros(1)
    ...
    with pytest.raises(ValueError, match="tensor names differ"):
        Florence2Pipeline.from_artifact(extra, weights_dir=DEFAULT_WEIGHTS_DIR)
    vision = tmp_path / "vision"
    shutil.copytree(artifact, vision)
    (vision / "manifest.json").write_text(json.dumps({**manifest, "tensors": [*manifest["tensors"], "model.vision_tower.blocks.0.x"]}))
    with pytest.raises(ValueError, match="last 4 decoder layers"):
        Florence2Pipeline.from_artifact(vision, weights_dir=DEFAULT_WEIGHTS_DIR)
